...
---
*Last updated: 2026-07-09 — v2.1 Hardening in progress (2/7 phases). Phase 30 (Secret Hygiene) complete + verified: HARD-SEC-01 — the OpenWeather `appid` can no longer escape into an error message, traceback, or log line. Root fix at both `client.py` raise sites (`fetch_onecall`/`geocode`) re-raises a redacted `httpx.HTTPStatusError(...) from None` (the `from None` drops the key-bearing `__context__` chain), plus a global `_LiveStderr.write` log backstop scrubbing `appid=<value>`→`appid=***` at the single stderr choke point shared by both `structlog.configure` sites; the locked type contract (`.response.status_code` intact for 6+ downstream branch sites) is preserved. New `weatherbot/_redact.py` helper + 4-test `tests/test_redact_hygiene.py` regression suite (capsys, all three leak paths + type canary). 810 tests green; code review 0 critical (2 latent warnings tracked in 30-REVIEW.md: re-raised `exc.request.url` still carries the raw key; `_LiveStderr.write` TypeErrors on bytes input — neither reachable today); Gate-1 self-UAT passed (SC1 driven live with a sentinel key, SC3 unit, SC2 partial); live `weatherbot.service` redeploy on `yahir-mint` is a deferred Gate-2 obligation. Builds on Phase 29 (startup validation & honest alerting).*

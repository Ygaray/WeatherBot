...
## Last Shipped Milestone: v2.1 Hardening

**Shipped 2026-07-17.** (No milestone currently active — define the next via `/gsd-new-milestone`.)

**Goal (delivered):** Fix the correctness defects surfaced by the whole-project audit so the briefing spine stops failing silently — no boot-green misconfig that drops briefings forever, no leaked OpenWeather key, no duplicate/mis-alerted sends, and correct timezone/date boundaries — then backfill the test gaps that let those bugs hide and sweep the latent/cleanup debt. **877 tests, ruff clean; 21/21 requirements validated; milestone audit passed; live `yahir-mint` Gate-2 verified.**
...
## Previously Shipped: v2.0 Bot Module Extraction ("The Great Decoupling")
...
All v2.1 requirements shipped and verified (21/21 — see milestones/v2.1-REQUIREMENTS.md; milestone audit passed, live `yahir-mint` Gate-2 driven 2026-07-17). Audit-driven hardening — every requirement is a testable correctness outcome traced to a whole-project-audit finding id; no new user surface:

- ✓ HARD-STARTUP-01 — v2.1 (Phase 29): daemon `run` boot enforces the same config/template validation as `check-config`, so a misconfig can't boot green and silently drop briefings.
- ✓ HARD-STARTUP-02 — v2.1 (Phase 29): a permanent config/template/empty-locations error is classified fatal (alerted + non-zero exit) instead of warn-looping forever as a fake transient network fault.
- ✓ HARD-STARTUP-03 — v2.1 (Phase 29): config→runtime startup ordering/logging corrected so a slot can't be silently omitted from the startup schedule announcement.
- ✓ HARD-SEC-01 — v2.1 (Phase 30): the OpenWeather `appid` never rides in an exception/traceback/log line — redacted at both client raise sites + a `_LiveStderr` backstop; the Discord inbound error path no longer dumps the key.
- ✓ HARD-DELIV-01 — v2.1 (Phase 31): post-send bookkeeping (`resolve_alert`/`stamp_success`) is log-and-swallow — a post-delivery DB error never releases the won claim (no duplicate briefing, no false `internal_error`) [F01, verified-first].
- ✓ HARD-DELIV-02 — v2.1 (Phase 31): forecast-slot delivery failures are detected (`DeliveryResult.ok`) and escalate the dead-slot streak instead of being counted as success.
- ✓ HARD-DELIV-03 — v2.1 (Phase 31): a delivery-only failure retries against the already-fetched payload (no fresh OpenWeather re-fetch on retry).
- ✓ HARD-DELIV-04 — v2.1 (Phase 31): a permanent Discord send auth failure (401/403) maps to the auth reason (redacted-URL carrier) rather than burning the full transient retry schedule.
- ✓ HARD-STORE-01 — v2.1 (Phase 31): SQLite opened `WAL` + `busy_timeout`, "read-only" reads take no write lock, so concurrent status reads and daemon writes no longer raise `database is locked`.
- ✓ HARD-STORE-02 — v2.1 (Phase 31): multi-step store writes are single-transaction atomic (no truncate-then-write / force-commit-before-insert corruption).
- ✓ HARD-TZ-01 — v2.1 (Phase 32): catch-up recovers a slot missed across local midnight — `plan_catchup` evaluates {today, yesterday-local} candidates keyed on the candidate day (F14).
- ✓ HARD-TZ-02 — v2.1 (Phase 32): the UV all-clear has window-end + past-peak hysteresis (no latch on a momentary sub-threshold dip) and the pre-warn↔crossing↔all-clear lifecycle leaves no never-fire gap (F15).
- ✓ HARD-TZ-03 — v2.1 (Phase 32): today's high/low, rain, UV window, and day-windows are computed against the configured IANA tz — `daily[0]`/positional indexing verifies the entry's own local date is today (F31/F35), hourly points sorted before interpolation (F32).
- ✓ HARD-TZ-04 — v2.1 (Phase 32): exactly one `_local_date_iso` implementation (new `weather/dates.py`) shared by `models.py`/`store.py`/`uvmonitor.py`, so rendered `{date}` and persisted `local_date` can't diverge.
- ✓ HARD-UI-01 — v2.1 (Phase 33): a bare `!weather`/`!sun`/`!wind`/`!alerts`/`!uv`/`!next-cloudy` resolves the default location app-side and replies correctly instead of crashing on `result=None` [F02, verified-first].
- ✓ HARD-UI-02 — v2.1 (Phase 33): a hot-reload landing mid-fetch no longer serves a stale cached result (generation guard), and the bounded `_PinnedTTLCache` can't evict the plain-weather entry; interaction races (invalidate-before-send, ack-before-mutate roll-back, selected-location reconcile) closed.
- ✓ HARD-UI-03 — v2.1 (Phase 33): rendered results are clean — deduped header, collapsed empty-token blanks, humanized local timestamps, dt-paired imperial/metric temps, unambiguous dated labels, and the default location marked on bare commands.
- ✓ HARD-TEST-01 — v2.1 (Phase 34): the false-green tests are corrected — the "concurrent double-fire" test exercises real barrier-synchronized concurrency, and heartbeat tick/success + naming assertions are strengthened so a never-delivering-but-healthy-looking daemon fails.
- ✓ HARD-TEST-02 — v2.1 (Phase 34): new regression tests cover the previously-uncovered high-risk paths (retry-then-alert exhaustion, catch-up across local midnight, rename-safe `id != name` through fire/catch-up/dedup, dt-based metric pairing, weekend roll-forward, store atomicity) — each correctness fix ships with a RED-first test.
- ✓ HARD-CLEAN-01 — v2.1 (Phase 35): the audit's dead/divergent code and inaccurate docs are removed or corrected (dead `-m` guard copy, discarded `is_transient` call, `emit_online`/`_do_reload` twins, dead `verbose` param, misleading docstrings), pinned by a negative-grep drift gate.
- ✓ HARD-CLEAN-02 — v2.1 (Phase 35): every remaining low-severity WeatherBot latent finding is fixed or carries an explicit in-code `# ACCEPTED (F##, v2.1)` annotation; the disposition ledger reconciles all 99 WB/BOTH findings and confirms the 17 hub findings routed out.

### Active

**No milestone active.** v2.1 Hardening shipped 2026-07-17; the next milestone is not yet defined. The v2.2 candidate scope is the 15 findings deferred out of v2.1 plus repinning onto hub `v0.1.2` once the 17 handed-off hub findings ship (human-gated tag cut).
...
**v2.1 Hardening — SHIPPED 2026-07-17 (all 7 phases complete + verified; milestone audit passed 21/21).** An audit-driven correctness/hardening pass over the whole-project audit: the briefing spine is hardened against its correctness defects — strict startup validation with honest fatal alerting (no boot-green misconfig dropping briefings), OpenWeather-key redaction, send atomicity/exactly-once + `WAL` persistence robustness (the F01 duplicate-briefing critical closed), timezone/date-boundary correctness (catch-up across local midnight, UV all-clear hysteresis, tz-anchored `daily[0]`), and panel/command robustness — plus real RED-first regression tests backfilling the paths the bugs hid in and a full cleanup sweep leaving no silent debt. **877 tests green, ruff clean.** Scope was 99 WeatherBot findings (65 FIXED + 19 ACCEPTED-with-rationale + 15 DEFERRED to a v2.2 hardening candidate); the **17 hub findings** route upstream in `HUB-FINDINGS-HANDOFF.md` and await a human-gated `YahirReusableBot` tag cut — WeatherBot repins after hub `v0.1.2`. Cross-repo jurisdiction held: every fix landed app-side with the pinned hub (`yahir_reusable_bot` v0.1.1) untouched. **Gate-2 live UAT PASSED on host `yahir-mint`** (2026-07-17): a clean v2.1 boot, no key leak in the journal, exactly-once held, correct timezone/date, and panel/commands confirmed — no deferred Gate-2 obligation remains. **Next:** define the next milestone via `/gsd-new-milestone` (v2.2 candidate: the 15 deferred findings + the hub `v0.1.2` repin).

**v2.0 Bot Module Extraction — SHIPPED 2026-07-07 (all 8 phases complete + verified; milestone audit passed 15/15).** The reusable bot core is physically extracted into its own PUBLIC repo `YahirReusableBot` (import root `yahir_reusable_bot`, `github.com/Ygaray/YahirReusableBot`, no console script); WeatherBot consumes it via a uv git **tag pin** (`[tool.uv.sources]` → `v0.1.1` @ `7f3cc00`, reproducible `uv.lock`) with an uncommitted venv editable overlay for local co-dev. Phase 28 closed the split: in-tree module removed, wheel collapsed to `["weatherbot"]`, `discord.py==2.7.1` pin moved into the module (inherited transitively), a startup provenance line announces the deployed sha (PEP 610 `direct_url.json` via `importlib.metadata`), and the `EXTENSION-GUIDE` + module GSD project + repin-ritual + promotion-ledger are stood up. **Gate-2 live UAT PASSED on host `yahir-mint`** (2026-07-07): restart against the pinned module + panel/reload/briefing/CLI all verified — and a live-only `on_message` recursion bug (broke `!panel`) was found and fixed *during* the UAT, shipped as module **v0.1.1** (`7f3cc00`), which the deploy is repinned to. Integration audit confirmed 6/6 module seams wired at the single composition root; retroactive security gate clean across phases 23–28 (`threats_open: 0`). All 15 requirements validated. _(v2.1 Hardening followed — see above.)_
...
| Audit-driven hardening milestone — every requirement is a testable outcome traced to a finding id; harden the existing One Call 3.0 daemon, no new surface (v2.1) | A whole-project audit surfaced correctness defects, not missing features; tying each requirement to a finding id keeps the milestone scoped to *fixing what's broken* rather than adding surface | ✓ v2.1 — 21 HARD-* requirements, each traced to a `WHOLE-PROJECT-REVIEW.md` finding; no new user-facing feature shipped |
| Correctness-first, cleanup-last sequencing (v2.1) | Stop the briefing spine failing silently *before* touching latent debt — a boot-green misconfig dropping every briefing outranks a rounding nit | ✓ v2.1 — startup(29)→secret(30)→send/persist(31)→tz(32)→panel(33)→test(34)→cleanup(35); each downstream phase assumes a validated boot |
| Pair DELIV + STORE in Phase 31 | The `WAL`/`busy_timeout` persistence hardening is the root de-risker of the F01 duplicate-briefing critical (the `database is locked`-after-delivery race), so they must land together | ✓ v2.1 (Phase 31) — store hardening + post-send log-and-swallow shipped in one phase; F01 closed |
| Verify-first on the two SWEEP-NEW criticals (v2.1) | F01 (`daemon.py`) and F02 (`dispatch.py`) were unverified sweep findings — reproducing the defect crash-first as a RED prevents "fixing" a non-bug or missing the real one | ✓ v2.1 — both reproduced RED before the fix landed (F01 Phase 31, F02 Phase 33) |
| Each correctness fix ships with a RED-first regression test (v2.1) | "Tests green" alone doesn't prove a false-green wasn't left in place; a test that fails against pre-fix behavior and passes against the fix is the real contract | ✓ v2.1 — formalized in Phase 34; every 29–33 fix carries a failing-first test |
| Hub findings human-gated — route the 17 hub findings out, don't fix here (v2.1) | Cutting a hub tag + repinning + deploying a live bot is a human-gated cross-repo step per ECOSYSTEM doctrine, not an autonomous milestone action | — Pending (cross-repo) — 17 findings in `HUB-FINDINGS-HANDOFF.md`; WeatherBot repins after hub `v0.1.2` |
| No-defer-low-findings honored (v2.1) | A hardening milestone shouldn't quietly defer low-impact findings; gap-closure pulled F38/F49/F50/F64/F81 out of DEFERRED into ACCEPTED/FIXED | ✓ v2.1 — remaining 15 DEFERRED are genuinely v2.2-scoped (medium/out-of-scope), ledger reconciles 65/19/15 |
...
---
*Last updated: 2026-07-17 after v2.1 Hardening milestone*

<details>
<summary>v2.1 pre-close phase-completion note (2026-07-13)</summary>

*v2.1 Hardening ALL 7 PHASES COMPLETE (29–35); ready for milestone close. Phase 35 (Cleanup Sweep) complete + verified (3/3 must-haves): HARD-CLEAN-01/02. The last v2.1 phase — swept the low/dead-code/latent residue behind the correctness work so the milestone leaves **no silent debt**. **Dead/divergent code removed (HARD-CLEAN-01, SC-1):** the dead `emit_online`/`_do_reload` daemon twins (F16 — confirmed dead via a live-path trace to the hub `reload_engine.service_pending()` + inlined online-ping, removed under a revert-gate; orphaned tests migrated to live seams, not blanket-deleted), the dead WB `_argv_is_weatherbot` `-m`-guard copy (F46 — removed, hub's live guard untouched), the dead `run_weather(verbose=)` param (F76), the discarded `is_transient(exc)` selfcheck call (F92); docstrings F66/F104 corrected/verified. A Wave-0 negative-grep gate (`tests/test_dead_code_removed.py`, tightened to `==0` post-removal per code-review WR-01) pins them gone. **Low-severity findings resolved-or-accepted (HARD-CLEAN-02, SC-2):** every remaining low WB finding is FIXED (F60 rounding, F68 non-JSON-2xx, F70 drop-beats-add, F74 HH:MM validator, F75 id-then-name resolve, F81 empty-Select guard, F105/F85 render, F78 dispatch guard — each with a regression test) or ACCEPTED with an in-code `# ACCEPTED (F##, v2.1)` annotation (19 total, incl. F67 kept as defense-in-depth after proving Phase-30 redaction doesn't cover the httpx stdlib-INFO path, and F71 Friday-as-weekend accepted+flagged-to-user). **Ledger reconciles (SC-3):** `WHOLE-PROJECT-REVIEW.md` §Disposition Ledger dispositions all 99 WB/BOTH findings — **65 FIXED + 19 ACCEPTED + 15 DEFERRED(v2.2) = 99** (the 15 deferred are medium/out-of-scope; a verifier gap that had wrongly deferred 5 LOW findings was closed by folding them in, per the no-defer-low-findings rule); the 17 hub findings confirmed routed to `HUB-FINDINGS-HANDOFF.md` (with the 17-vs-18 note). **877 tests green** (exit 0; the "2 snapshots failed" line is the known syrupy quirk). Hub boundary held (zero `yahir_reusable_bot/` edits). Post-execution code review: 0 critical, 2 warning, 3 info — WR-01 (drift-gate `<=1`→`==0`) applied; the rest advisory. 3 pre-existing daemon.py ruff nits documented as out-of-audit-scope follow-ups. Next: `/gsd-complete-milestone` to close v2.1 (Gate-2 human UAT + milestone audit).*

</details>

<details>
<summary>Phase 33 (Interactive & Panel Robustness) — complete + verified (18/18): HARD-UI-01/02/03</summary>

...
---
*Last updated: 2026-07-13 — v2.1 Hardening in progress (6/7 phases). Phase 34 (Test-Gap Backfill) complete + verified (8/8 must-haves): HARD-TEST-01/02. A **tests-only** phase closing the coverage that let the audit bugs hide — zero production and zero hub change (`git diff` on `weatherbot/` empty; the D-07 F109 watchpoint resolved GREEN, so `weather/dates.py` was never touched). **False-greens corrected (HARD-TEST-01):** the "concurrent" double-fire test (`test_scheduler.py`) now runs two `threading.Barrier(2)`-synced real threads racing one file-backed `tmp_db`, plus a **meta-guard** that monkeypatches a weakened SELECT-then-INSERT `claim_slot` and proves it then double-delivers (F106); heartbeat now asserts `last_success_utc is None` after a bare tick (F114); the within-burst wait bound is the **derived** `step..step*1.5` (~85.71..128.57 from `BURST_SPREAD_S/(BURST_SIZE-1)`), not the loose `< 150.0` literal (F112); the cache test uses a distinct `id != name` (F115); reconcile asserts register-before-remove ordering (F116). **Missing coverage added (HARD-TEST-02):** rename-safe `id != name` through `fire_slot`/`plan_catchup`/alert-dedup (F108), Retry-After 429 on the mid-pause attempt collapsing to the 120s cap (F110), today-not-at-`daily[0]` selection (F109), weekend whole-block roll-forward + null-`dt` skip (F111/F113), store both-or-neither transactional rollback + WAL (F37/F63) — with F14 (midnight catch-up), F107 (dt-pairing), F01 (post-send re-fire) cited **[EXISTS]** for the SC-3 ledger. Several of the highest-risk pins carry a documented D-06 red→green spot-check (weaken the fix → test bites → restore). **878 tests green** (exit 0; the "2 snapshots failed" line is the known syrupy quirk). Post-execution code review: 0 critical, 3 warning, 4 info — all test-robustness/consistency nits (barrier-timeout vs `busy_timeout` collision, global `threading.Event` patch blast radius, `<=`-vs-`<` bound inconsistency), advisory and folded to Phase 35 or an optional `/gsd-code-review 34 --fix`. Cross-repo jurisdiction held. Builds on the fixes from Phases 29–33 that these tests pin. Next: Phase 35 (Cleanup Sweep) — the last v2.1 phase.*

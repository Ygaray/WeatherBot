...
- [ ] **HARD-DELIV-02**: Forecast-slot delivery failures are detected and alerted rather than silently swallowed (F08, forecast-slot fire path).
...
- [ ] **HARD-TZ-03**: `daily[0]` (and any positional daily/hourly indexing) is anchored to the configured location IANA timezone rather than positionally/UTC, so today's high/low and forecast day-windows are correct across DST and near midnight (F31, F35, F91, F109, and sibling tz findings).
